...
proc ::elixir::elixirgui {} {
	...
	#set some bottons for the function
	frame $t.bot
	button $t.bot.s -text Sumbit -command ::elixir::call_python
	button $t.bot.re -text Reset -command ::elixir::resetelements
	button $t.bot.ra -text "Random numbers" -command ::elixir::randomfill
	pack $t.bot.ra -side top -pady 2 -fill x
	pack $t.bot.re -side top -pady 2 -fill x
	pack $t.bot.s -side top -pady 2 -fill x
	pack $t.bot -side top -pady 2 -fill x


proc ::elixir::resetelements {} \
{
	variable w
	variable pnum
	variable pnum2
	for {set i 1} {$i <= 20} {incr i} {
		for {set j 1} {$j <= 3} {incr j} {
			set ::elixir::molsx$j$i 0.000
			set ::elixir::molsy$j$i 0.000
			set ::elixir::molsz$j$i 0.000
		}
...
proc ::elixir::randomfill {} \
{
	...
	for {set i 1} {$i <= 20} {incr i} {
		for {set j 1} {$j <= 3} {incr j} {
			set ::elixir::molsx$j$i [expr { rand()*10 }]
			set ::elixir::molsy$j$i [expr { rand()*10 }]
			set ::elixir::molsz$j$i [expr { rand()*10 }]
		}
...
proc ::elixir::packmols {} \
{
	...
	if {$pnum>=5} {
		wm geometry $t 640x[expr $pnum*120]+300+50
	} else {
		wm geometry $t 640x640+300+50
	}
}
proc ::elixir::packmols2 {} \
{
	...
	if {$pnum2>=5} {
		wm geometry $t 640x[expr $pnum2*120]+300+50
	} else {
		wm geometry $t 640x640+300+50
	}
}
...
#test on 
# set output [exec python "test.py" [list $::elixir::molsx11 $::elixir::molsy11 $::elixir::molsz11 \
...
set output [exec python [file join $::env(ELIXIRDIR) "test.py"] [list $::elixir::molsx11 $::elixir::molsy11 $::elixir::molsz11 \
	$::elixir::molsx12 $::elixir::molsy12 $::elixir::molsz12 \
	$::elixir::molsx13 $::elixir::molsy13 $::elixir::molsz13 \
	$::elixir::molsx14 $::elixir::molsy14 $::elixir::molsz14 \
	$::elixir::molsx15 $::elixir::molsy15 $::elixir::molsz15 \
